...
exports.parse = function parse(argv) {
  ...
  }).usage("$0 <path> [options]").example("Build a marko file", "$0 component.marko").example("Build the current directory", "$0 .").validate(function (result) {
    if (result.version) {
...
exports.run = options => {
  ...
  compiler.run(async (err, multiStats) => {
    ...
    if (err) {
      ...
      console.log(multiStats.toString({
        preset: "minimal",
        colors: true
      }));
      console.log("\nBuild complete");
